...
const Marketplace = () => {
  ...
  return (
    <div>
      ...
      {/* FLEX CONTAINER: Filters and Marketplace */}
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            <div style={{ fontWeight: "bold", fontSize: "1.1rem" }}>Auction Price</div>
            <RangeSlider />
            <input
              type="range"
              min="0"
            ...
            />
            <div style={{ fontSize: "0.9rem" }}>{maxPriceAuction} Coins</div>
          </div>

          {/* HP Section */}
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            <div style={{ fontWeight: "bold", fontSize: "1.1rem" }}>HP</div>
            <input
              type="range"
              min="0"
              max="1000"
            ...
            />
            <div style={{ fontSize: "0.9rem" }}>{minPriceHP} HP</div>
          </div>
                  ...
                  <div className="overlays">
                    ...
                    <div className="status">
                        ...
                        <div>
                          ...
                          <ul>
                            {item.card.abilities.map((ability, index) => (
                              <li key={index}>{ability.name}: {ability.text}</li>
                            ))}
                          </ul>
                        </div>
                      )}
                      ...
                      {item.card.weaknesses && item.card.weaknesses.length > 0 && (
                        <p>Weaknesses: {item.card.weaknesses.map(w => `${w.type} ${w.value}`).join(", ")}</p>
                      )}
                      {item.card.resistances && item.card.resistances.length > 0 && (
                        <p>Resistances: {item.card.resistances.map(r => `${r.type} ${r.value}`).join(", ")}</p>
                      )}
                      {item.card.set_data && (
                        <p>Set: {item.card.set_data.name} ({item.card.set_data.series})</p>
                      )}
